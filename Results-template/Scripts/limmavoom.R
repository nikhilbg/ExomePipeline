...
#print(x)
#print(design)
png("Limma_VoomPlot.png")
v1 <- voom(as.matrix(x),design,plot=TRUE,normalize="quantile")
dev.off()
sf = v1$E/log2((x/colSums(x))*1000000)
write.table(sf,file="Limma_Voom_scaling_factors.txt",row.names=TRUE,col.names=TRUE,sep="\t",quote=FALSE)
png("Limma_HistLimmavoomNormFilter.png")
...
png(paste("Limma_",cons[i],"_MAplot_Limma_v2.png",sep=""))
